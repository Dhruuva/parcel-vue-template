...
})({"a6ObB":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "0627dd35813167df";
...
},{}],"75iMF":[function(require,module,exports) {
...
var _entryVue = require("./components/Entry.vue");
var _entryVueDefault = parcelHelpers.interopDefault(_entryVue);
var _cbtnVue = require("./components/cbtn.vue");
var _cbtnVueDefault = parcelHelpers.interopDefault(_cbtnVue);
var _mainCss = require("./assets/main.css");
globalThis.__VUE_OPTIONS_API__ = true;
globalThis.__VUE_PROD_DEVTOOLS__ = false;
const app = _vueEsmBundler.createApp({});
app.component("my-header", _headerVueDefault.default);
app.component("my-entry", _entryVueDefault.default);
app.component("my-button", _cbtnVueDefault.default);
app.mount("#app1");

},{"vue/dist/vue.esm-bundler":"gs3Mk","./components/header.vue":"341XZ","./components/Entry.vue":"lV0KE","./components/cbtn.vue":"3fsRG","./assets/main.css":"7SfAk","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gs3Mk":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
function compileToFunction(template, options) {
    if (!_shared.isString(template)) {
        if (template.nodeType) template = template.innerHTML;
        else {
            _runtimeDom.warn(`invalid template option: `, template);
            return _shared.NOOP;
        }
    }
    const key = template;
    ...
    if (template[0] === '#') {
        ...
        if (!el) _runtimeDom.warn(`Template element not found or is empty: ${template}`);
        // __UNSAFE__
    ...
    const { code  } = _compilerDom.compile(template, _shared.extend({
        hoistStatic: true,
        onError: onError,
    ...
    function onError(err, asWarning = false) {
        ...
        const codeFrame = err.loc && _shared.generateCodeFrame(template, err.loc.start.offset, err.loc.end.offset);
        _runtimeDom.warn(codeFrame ? `${message}\n${codeFrame}` : message);
    }
...
_runtimeDom.registerRuntimeCompiler(compileToFunction);
...
_compilerCore.registerRuntimeHelpers({
...
const isRawTextContainer = /*#__PURE__*/ _shared.makeMap('style,iframe,script,noscript', true);
const parserOptions = {
    isVoidTag: _shared.isVoidTag,
    isNativeTag: (tag)=>_shared.isHTMLTag(tag) || _shared.isSVGTag(tag)
    ,
    isPreTag: (tag)=>tag === 'pre'
    ,
    decodeEntities: decodeHtmlBrowser,
    isBuiltInComponent: (tag)=>{
        if (_compilerCore.isBuiltInType(tag, `Transition`)) return TRANSITION;
        else if (_compilerCore.isBuiltInType(tag, `TransitionGroup`)) return TRANSITION_GROUP;
    },
...
const transformStyle = (node)=>{
    if (node.type === 1 /* ELEMENT */ ) node.props.forEach((p, i)=>{
        ...
        node.props[i] = {
            type: 7 /* DIRECTIVE */ ,
            name: `bind`,
            arg: _compilerCore.createSimpleExpression(`style`, true, p.loc),
...
const parseInlineCSS = (cssText, loc)=>{
    const normalized = _shared.parseStringStyle(cssText);
    return _compilerCore.createSimpleExpression(JSON.stringify(normalized), false, loc, 3 /* CAN_STRINGIFY */ );
};
function createDOMCompilerError(code, loc) {
    return _compilerCore.createCompilerError(code, loc, DOMErrorMessages);
}
...
const transformVHtml = (dir, node, context)=>{
    ...
    return {
        props: [
            _compilerCore.createObjectProperty(_compilerCore.createSimpleExpression(`innerHTML`, true, loc), exp || _compilerCore.createSimpleExpression('', true))
        ]
    };
};
const transformVText = (dir, node, context)=>{
    ...
    return {
        props: [
            _compilerCore.createObjectProperty(_compilerCore.createSimpleExpression(`textContent`, true), exp ? _compilerCore.createCallExpression(context.helperString(_compilerCore.TO_DISPLAY_STRING), [
                exp
            ], loc) : _compilerCore.createSimpleExpression('', true))
        ]
    };
};
const transformModel = (dir, node, context)=>{
    const baseResult = _compilerCore.transformModel(dir, node, context);
    ...
    function checkDuplicatedValue() {
        const value = _compilerCore.findProp(node, 'value');
    ...
    if (tag === 'input' || tag === 'textarea' || tag === 'select' || isCustomElement) {
        ...
        if (tag === 'input' || isCustomElement) {
            const type = _compilerCore.findProp(node, `type`);
            if (type) {
            ...
            } else if (_compilerCore.hasDynamicKeyVBind(node)) // element has bindings with dynamic keys, which can possibly contain
            // "type".
            directiveToUse = V_MODEL_DYNAMIC;
...
const isEventOptionModifier = /*#__PURE__*/ _shared.makeMap(`passive,once,capture`);
const isNonKeyModifier = /*#__PURE__*/ _shared.makeMap(// event propagation management
`stop,prevent,self,` + // system modifiers + exact
`ctrl,shift,alt,meta,exact,` + // mouse
`middle`);
// left & right could be mouse or key modifiers based on event type
const maybeKeyModifier = /*#__PURE__*/ _shared.makeMap('left,right');
const isKeyboardEvent = /*#__PURE__*/ _shared.makeMap(`onkeyup,onkeydown,onkeypress`, true);
const resolveModifiers = (key, modifiers, context, loc)=>{
    ...
    for(let i = 0; i < modifiers.length; i++){
        ...
        if (modifier === 'native' && _compilerCore.checkCompatEnabled("COMPILER_V_ON_NATIVE" /* COMPILER_V_ON_NATIVE */ , context, loc)) eventOptionModifiers.push(modifier);
        ...
        eventOptionModifiers.push(modifier);
            ...
            if (maybeKeyModifier(modifier)) {
                if (_compilerCore.isStaticExp(key)) {
...
const transformClick = (key, event)=>{
    const isStaticClick = _compilerCore.isStaticExp(key) && key.content.toLowerCase() === 'onclick';
    return isStaticClick ? _compilerCore.createSimpleExpression(event, true) : key.type !== 4 /* SIMPLE_EXPRESSION */  ? _compilerCore.createCompoundExpression([
        `(`,
        key,
        `) === "onClick" ? "${event}" : (`,
        key,
...
const transformOn = (dir, node, context)=>{
    return _compilerCore.transformOn(dir, node, context, (baseResult)=>{
        const { modifiers  } = dir;
        ...
        if (nonKeyModifiers.length) handlerExp = _compilerCore.createCallExpression(context.helper(V_ON_WITH_MODIFIERS), [
            handlerExp,
            JSON.stringify(nonKeyModifiers)
        ]);
        if (keyModifiers.length && (!_compilerCore.isStaticExp(key) || isKeyboardEvent(key.content))) handlerExp = _compilerCore.createCallExpression(context.helper(V_ON_WITH_KEYS), [
            handlerExp,
            JSON.stringify(keyModifiers)
        ]);
        if (eventOptionModifiers.length) {
            const modifierPostfix = eventOptionModifiers.map(_shared.capitalize).join('');
            key = _compilerCore.isStaticExp(key) ? _compilerCore.createSimpleExpression(`${key.content}${modifierPostfix}`, true) : _compilerCore.createCompoundExpression([
                `(`,
                key,
                `) + "${modifierPostfix}"`
            ]);
        ...
        return {
            props: [
                _compilerCore.createObjectProperty(key, handlerExp)
            ]
        };
    });
};
...
function compile(template, options = {}) {
    return _compilerCore.baseCompile(template, _shared.extend({}, parserOptions, options, {
        nodeTransforms: [
        ...
        directiveTransforms: _shared.extend({}, DOMDirectiveTransforms, options.directiveTransforms || {}),
        transformHoist: null
    }));
}
function parse(template, options = {}) {
    return _compilerCore.baseParse(template, _shared.extend({}, parserOptions, options));
}
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "generateCodeFrame", ()=>_shared.generateCodeFrame
);
...
function createObjectProperty(key, value) {
    return {
        type: 16 /* JS_PROPERTY */ ,
        loc: locStub,
        key: _shared.isString(key) ? createSimpleExpression(key, true) : key,
        value
    };
}
...
function createInterpolation(content, loc) {
    return {
        ...
        loc,
        content: _shared.isString(content) ? createSimpleExpression(content, false, loc) : content
    };
}
...
;
const isBuiltInType = (tag, expected)=>tag === expected || tag === _shared.hyphenate(expected)
;
...
function advancePositionWithClone(pos, source, numberOfCharacters = source.length) {
    return advancePositionWithMutation(_shared.extend({}, pos), source, numberOfCharacters);
}
...
function findDir(node, name, allowEmpty = false) {
    for(let i = 0; i < node.props.length; i++){
        ...
        if (p.type === 7 /* DIRECTIVE */  && (allowEmpty || p.exp) && (_shared.isString(name) ? p.name === name : name.test(p.name))) return p;
    }
}
...
function getUnnormalizedProps(props, callPath = []) {
    if (props && !_shared.isString(props) && props.type === 14 /* JS_CALL_EXPRESSION */ ) {
        const callee = props.callee;
        if (!_shared.isString(callee) && propsHelperSet.has(callee)) return getUnnormalizedProps(props.arguments[0], callPath.concat(props));
    }
    return [
        props,
        callPath
...
function injectProp(node, prop, context) {
    ...
    if (props == null || _shared.isString(props)) propsWithInjection = createObjectExpression([
        prop
    ]);
        ...
        if (!_shared.isString(first) && first.type === 15 /* JS_OBJECT_EXPRESSION */ ) first.properties.unshift(prop);
...
function hasScopeRef(node, ids) {
    ...
    switch(node.type){
        ...
        case 8 /* COMPOUND_EXPRESSION */ :
            return node.children.some((c)=>_shared.isObject(c) && hasScopeRef(c, ids)
            );
...
function createParserContext(content, rawOptions) {
    const options = _shared.extend({}, defaultParserOptions);
    let key;
...
function parseChildren(context, mode, ancestors) {
    ...
    while(!isEnd(context, mode, ancestors)){
        ...
        if (_shared.isArray(node)) for(let i = 0; i < node.length; i++)pushNode(nodes, node[i]);
...
const isSpecialTemplateDirective = /*#__PURE__*/ _shared.makeMap(`if,else,else-if,for,slot`);
...
function isComponent(tag, props, context) {
    ...
    for(let i = 0; i < props.length; i++){
        ...
        if (p.type === 6 /* ATTRIBUTE */ ) {
            ...
            p.name === 'bind' && isStaticArgOf(p.arg, 'is') && true && checkCompatEnabled("COMPILER_IS_ON_ELEMENT" /* COMPILER_IS_ON_ELEMENT */ , context, p.loc)) return true;
        }
    }
}
 ...
 */ function parseTextData(context, length, mode) {
    ...
    if (mode === 2 /* RAWTEXT */  || mode === 3 /* CDATA */  || !rawText.includes('&')) return rawText;
...
function walk(node, context, doNotHoistNode = false) {
    ...
    if (hoistedCount && hoistedCount === originalCount && node.type === 1 /* ELEMENT */  && node.tagType === 0 /* ELEMENT */  && node.codegenNode && node.codegenNode.type === 13 /* VNODE_CALL */  && _shared.isArray(node.codegenNode.children)) node.codegenNode.children = context.hoist(createArrayExpression(node.codegenNode.children));
}
function getConstantType(node, context) {
    ...
    switch(node.type){
        case 1 /* ELEMENT */ :
            ...
            if (codegenNode.isBlock && node.tag !== 'svg' && node.tag !== 'foreignObject') return 0 /* NOT_CONSTANT */ ;
            const flag = getPatchFlag(codegenNode);
            if (!flag) {
        ...
        case 8 /* COMPOUND_EXPRESSION */ :
            ...
            for(let i = 0; i < node.children.length; i++){
                ...
                if (_shared.isString(child) || _shared.isSymbol(child)) continue;
...
function getConstantTypeOfHelperCall(value, context) {
    if (value.type === 14 /* JS_CALL_EXPRESSION */  && !_shared.isString(value.callee) && allowHoistedHelperSet.has(value.callee)) {
...
function createTransformContext(root, { filename ='' , prefixIdentifiers =false , hoistStatic: hoistStatic1 = false , cacheHandlers =false , nodeTransforms =[] , directiveTransforms ={} , transformHoist =null , isBuiltInComponent =_shared.NOOP , isCustomElement =_shared.NOOP , expressionPlugins =[] , scopeId =null , slotted =true , ssr =false , inSSR =false , ssrCssVars =`` , bindingMetadata =_shared.EMPTY_OBJ , inline =false , isTS =false , onError =defaultOnError , onWarn =defaultOnWarn , compatConfig  }) {
    ...
    const context = {
        // options
        selfName: nameMatch && _shared.capitalize(_shared.camelize(nameMatch[1])),
        prefixIdentifiers,
        ...
        hoist (exp) {
            if (_shared.isString(exp)) exp = createSimpleExpression(exp);
...
function createRootCodegen(root, context) {
    ...
    if (children.length === 1) {
        ...
        let patchFlagText = _shared.PatchFlagNames[64 /* STABLE_FRAGMENT */ ];
        // check if the fragment actually contains a single valid child with
        // the rest being comments
        if (children.filter((c)=>c.type !== 3 /* COMMENT */ 
        ).length === 1) {
            patchFlag |= 2048 /* DEV_ROOT_FRAGMENT */ ;
            patchFlagText += `, ${_shared.PatchFlagNames[2048 /* DEV_ROOT_FRAGMENT */ ]}`;
        }
...
function traverseChildren(parent, context) {
    ...
    for(; i < parent.children.length; i++){
        ...
        if (_shared.isString(child)) continue;
...
function traverseNode(node, context) {
    ...
    for(let i = 0; i < nodeTransforms.length; i++){
        ...
        if (onExit) {
            if (_shared.isArray(onExit)) exitFns.push(...onExit);
...
function createStructuralDirectiveTransform(name, fn) {
    const matches = _shared.isString(name) ? (n)=>n === name
     : (n)=>name.test(n)
    ;
    return (node, context)=>{
...
function isText$1(n) {
    return _shared.isString(n) || n.type === 4 /* SIMPLE_EXPRESSION */  || n.type === 2 /* TEXT */  || n.type === 5 /* INTERPOLATION */  || n.type === 8 /* COMPOUND_EXPRESSION */ ;
}
function genNodeListAsArray(nodes, context) {
    const multilines = nodes.length > 3 || nodes.some((n)=>_shared.isArray(n) || !isText$1(n)
    );
    context.push(`[`);
...
function genNodeList(nodes, context, multilines = false, comma = true) {
    ...
    for(let i = 0; i < nodes.length; i++){
        const node = nodes[i];
        if (_shared.isString(node)) push(node);
        else if (_shared.isArray(node)) genNodeListAsArray(node, context);
...
function genNode(node, context) {
    if (_shared.isString(node)) {
        context.push(node);
        return;
    }
    if (_shared.isSymbol(node)) {
...
function genCompoundExpression(node, context) {
    for(let i = 0; i < node.children.length; i++){
        const child = node.children[i];
        if (_shared.isString(child)) context.push(child);
...
function genCallExpression(node, context) {
    const { push , helper , pure  } = context;
    const callee = _shared.isString(node.callee) ? node.callee : helper(node.callee);
    if (pure) push(PURE_ANNOTATION);
    push(callee + `(`, node);
...
function genFunctionExpression(node, context) {
    ...
    push(`(`, node);
    if (_shared.isArray(params)) genNodeList(params, context);
    ...
    if (returns) {
        ...
        if (_shared.isArray(returns)) genNodeListAsArray(returns, context);
    ...
    if (dir.name === 'if') {
        ...
        while(i-- >= -1){
            ...
            if (sibling && sibling.type === 9 /* IF */ ) {
                ...
                if (dir.name === 'else-if' && sibling.branches[sibling.branches.length - 1].condition === undefined) context.onError(createCompilerError(30 /* X_V_ELSE_NO_ADJACENT_IF */ , node.loc));
...
const transformFor = createStructuralDirectiveTransform('for', (node, dir, context)=>{
    ...
    return processFor(node, dir, context, (forNode)=>{
        ...
        forNode.codegenNode = createVNodeCall(context, helper(FRAGMENT), undefined, renderExp, fragmentFlag + ` /* ${_shared.PatchFlagNames[fragmentFlag]} */`, undefined, undefined, true, !isStableFragment /* disableTracking */ , false, node.loc);
        return ()=>{
            ...
            childBlock = createVNodeCall(context, helper(FRAGMENT), keyProperty ? createObjectExpression([
                keyProperty
            ]) : undefined, node.children, 64 /* STABLE_FRAGMENT */  + ` /* ${_shared.PatchFlagNames[64 /* STABLE_FRAGMENT */ ]} */`, undefined, undefined, true, undefined, false);
            else {
...
function buildSlots(node1, context, buildSlotFn = buildClientSlotFn) {
    ...
    createSimpleExpression(slotFlag + ` /* ${_shared.slotFlagsText[slotFlag]} */`, false))), loc);
...
const transformElement = (node, context)=>{
    ...
    return function postTransformElement() {
        ...
        const isDynamicComponent = _shared.isObject(vnodeTag) && vnodeTag.callee === RESOLVE_DYNAMIC_COMPONENT;
        let vnodeProps;
        let vnodeChildren;
        let vnodePatchFlag;
        let patchFlag = 0;
        ...
        isDynamicComponent || vnodeTag === TELEPORT || vnodeTag === SUSPENSE || !isComponent6 && (tag === 'svg' || tag === 'foreignObject');
        // props
        if (props.length > 0) {
        ...
        if (patchFlag !== 0) {
            {
                ...
                vnodePatchFlag = patchFlag + ` /* ${_shared.PatchFlagNames[patchFlag]} */`;
                    ...
                    // bitwise flags
                    const flagNames = Object.keys(_shared.PatchFlagNames).map(Number).filter((n)=>n > 0 && patchFlag & n
                    ).map((n)=>_shared.PatchFlagNames[n]
                    ).join(`, `);
...
function buildProps(node, context, props = node.props, ssr = false) {
    ...
    const analyzePatchFlag = ({ key , value  })=>{
        if (isStaticExp(key)) {
            ...
            const isEventHandler = _shared.isOn(name);
            if (!isComponent7 && isEventHandler && // omit the flag for click handlers because hydration gives click
            // dedicated fast path.
            name.toLowerCase() !== 'onclick' && // omit v-model handlers
            name !== 'onUpdate:modelValue' && // omit onVnodeXXX hooks
            !_shared.isReservedProp(name)) hasHydrationEventBinding = true;
            if (isEventHandler && _shared.isReservedProp(name)) hasVnodeHook = true;
    ...
    for(let i = 0; i < props.length; i++){
        ...
        if (prop.type === 6 /* ATTRIBUTE */ ) {
            ...
            if (name === 'is' || isVBind && isStaticArgOf(arg1, 'is') && (isComponentTag(tag) || isCompatEnabled("COMPILER_IS_ON_ELEMENT" /* COMPILER_IS_ON_ELEMENT */ , context))) continue;
            // skip v-on in SSR compilation
            if (isVOn && ssr) continue;
            if (// #938: elements with dynamic keys should be forced into blocks
            isVBind && isStaticArgOf(arg1, 'key') || isVOn && hasChildren && isStaticArgOf(arg1, 'vue:before-update')) shouldUseBlock = true;
            if (isVBind && isStaticArgOf(arg1, 'ref') && context.scopes.vFor > 0) properties.push(createObjectProperty(createSimpleExpression('ref_for', true), createSimpleExpression('true')));
                    ...
                    if (isVBind) {
                        {
                            const hasOverridableKeys = mergeArgs.some((arg)=>{
                                if (arg.type === 15 /* JS_OBJECT_EXPRESSION */ ) return arg.properties.some(({ key  })=>{
                                    ...
                                    return key.content !== 'class' && key.content !== 'style' && !_shared.isOn(key.content);
                                });
            ...
            if (directiveTransform) {
                ...
                if (needRuntime) {
                    ...
                    if (_shared.isSymbol(needRuntime)) directiveImportMap.set(prop, needRuntime);
                }
            } else if (!_shared.isBuiltInDirective(name)) {
    ...
    if (!shouldUseBlock && (patchFlag === 0 || patchFlag === 32 /* HYDRATE_EVENTS */ ) && (hasRef || hasVnodeHook || runtimeDirectives.length > 0)) patchFlag |= 512 /* NEED_PATCH */ ;
...
function dedupeProperties(properties) {
    ...
    for(let i = 0; i < properties.length; i++){
        ...
        if (existing) {
            if (name === 'style' || name === 'class' || _shared.isOn(name)) mergeAsArray(existing, prop);
        } else {
...
const transformOn = (dir, node, context, augmentor)=>{
    ...
    if (arg.type === 4 /* SIMPLE_EXPRESSION */ ) {
        if (arg.isStatic) {
            ...
            eventName = createSimpleExpression(_shared.toHandlerKey(_shared.camelize(rawName)), true, arg.loc);
        } else // #2388
...
const transformBind = (dir, _node, context)=>{
    ...
    if (modifiers.includes('camel')) {
        if (arg.type === 4 /* SIMPLE_EXPRESSION */ ) {
            if (arg.isStatic) arg.content = _shared.camelize(arg.content);
    ...
    return ()=>{
        ...
        for(let i7 = 0; i7 < children.length; i7++){
            ...
            if (isText(child) || child.type === 8 /* COMPOUND_EXPRESSION */ ) {
                ...
                if (!context.ssr && getConstantType(child, context) === 0 /* NOT_CONSTANT */ ) callArgs.push(1 /* TEXT */  + ` /* ${_shared.PatchFlagNames[1 /* TEXT */ ]} */`);
                children[i7] = {
                    type: 12 /* TEXT_CALL */ ,
                    content: child,
                    loc: child.loc,
...
function baseCompile(template, options = {}) {
    ...
    const ast = _shared.isString(template) ? baseParse(template, options) : template;
    const [nodeTransforms, directiveTransforms] = getBaseTransformPreset();
    transform(ast, _shared.extend({}, options, {
        prefixIdentifiers,
        nodeTransforms: [
            ...nodeTransforms,
            ...options.nodeTransforms || [] // user transforms
        ],
        directiveTransforms: _shared.extend({}, directiveTransforms, options.directiveTransforms || {} // user transforms
        )
    }));
    return generate(ast, _shared.extend({}, options, {
        prefixIdentifiers
    }));
}
...
},{"@vue/shared":"3SM3y","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"341XZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let script;
let initialize = ()=>{
    ...
    script.__scopeId = 'data-v-0393db';
    script.__file = "/home/vaman/apps/2022/parcelApp/src/components/header.vue";
};
initialize();
if (module.hot) {
    script.__hmrId = '0393db-hmr';
    module.hot.accept(()=>{
        setTimeout(()=>{
            initialize();
            if (!__VUE_HMR_RUNTIME__.createRecord('0393db-hmr', script)) __VUE_HMR_RUNTIME__.reload('0393db-hmr', script);
        }, 0);
    });
}
exports.default = script;

},{"script:./header.vue":"7naKh","template:./header.vue":"53GNG","style:./header.vue":"bFWqG","custom:./header.vue":"4YZm1","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7naKh":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _vue = require("@iconify/vue");
exports.default = {
    props: {
        active: {
            type: String,
            ...
            stuff: [
                {
                    ...
                    isActive: true,
                    name: "Home",
                    ico: "ic:baseline-account-balance"
                },
                {
                    id: 2,
                    page: "about.html",
                    name: "About",
                    ico: "ic:baseline-contact-support"
                },
                {
                    id: 3,
                    page: "market.html",
                    name: "Market",
                    ico: "ic:sharp-barcode"
                },
                {
                    id: 4,
                    page: "sakura.html",
                    name: "Sakura",
                    ico: "ic:round-compost"
                }, 
            ]
        };
    },
...
},{"@iconify/vue":"d1SnY","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"d1SnY":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
parcelHelpers.export(exports, "replaceIDs", ()=>replaceIDs
);
parcelHelpers.export(exports, "shareStorage", ()=>shareStorage
);
...
function mergeIconData(icon, alias) {
    ...
    for(const key in iconDefaults){
        const prop1 = key;
        if (alias[prop1] !== void 0) {
            const value = alias[prop1];
            if (result[prop1] === void 0) {
                result[prop1] = value;
                continue;
            }
            switch(prop1){
                case "rotate":
                    result[prop1] = (result[prop1] + value) % 4;
                    break;
                case "hFlip":
                case "vFlip":
                    result[prop1] = value !== result[prop1];
                    break;
                default:
                    result[prop1] = value;
            }
...
function getIconData$1(data, name, full = false) {
    function getIcon1(name2, iteration) {
        if (data.icons[name2] !== void 0) return Object.assign({}, data.icons[name2]);
        if (iteration > 5) return null;
        const aliases = data.aliases;
        if (aliases && aliases[name2] !== void 0) {
            const item = aliases[name2];
            const result2 = getIcon1(item.parent, iteration + 1);
            if (result2) return mergeIconData(result2, item);
            return result2;
        }
        const chars = data.chars;
        if (!iteration && chars && chars[name2] !== void 0) return getIcon1(chars[name2], iteration + 1);
        return null;
    }
    const result = getIcon1(name, 0);
    if (result) {
        for(const key in iconDefaults)if (result[key] === void 0 && data[key] !== void 0) result[key] = data[key];
    }
    return result && full ? fullIcon(result) : result;
}
function isVariation(item) {
    for(const key in iconDefaults){
        if (item[key] !== void 0) return true;
    }
    return false;
}
function parseIconSet(data, callback, options) {
    ...
    if (parseAliases !== "none" && typeof data.aliases === "object") {
        ...
        Object.keys(aliases).forEach((name)=>{
            if (parseAliases === "variations" && isVariation(aliases[name])) return;
            const iconData = getIconData$1(data, name, true);
            if (iconData) {
                callback(name, iconData);
                names.push(name);
...
for(const prop in iconDefaults)optionalProperties[prop] = typeof iconDefaults[prop];
function quicklyValidateIconSet(obj) {
    if (typeof obj !== "object" || obj === null) return null;
    const data = obj;
    if (typeof data.prefix !== "string" || !obj.icons || typeof obj.icons !== "object") return null;
    for(const prop2 in optionalProperties){
        if (obj[prop2] !== void 0 && typeof obj[prop2] !== optionalProperties[prop2]) return null;
    }
    const icons = data.icons;
    for(const name in icons){
        const icon = icons[name];
        if (!name.match(matchName) || typeof icon.body !== "string") return null;
        for(const prop3 in iconDefaults){
            if (icon[prop3] !== void 0 && typeof icon[prop3] !== typeof iconDefaults[prop3]) return null;
        }
    }
    const aliases = data.aliases;
    if (aliases) for(const name1 in aliases){
        const icon = aliases[name1];
        const parent = icon.parent;
        if (!name1.match(matchName) || typeof parent !== "string" || !icons[parent] && !aliases[parent]) return null;
        for(const prop4 in iconDefaults){
            if (icon[prop4] !== void 0 && typeof icon[prop4] !== typeof iconDefaults[prop4]) return null;
        }
    }
    return data;
}
...
try {
    const w = window || self;
    if (w && w._iconifyStorage.version === storageVersion) storage$1 = w._iconifyStorage.storage;
} catch (err) {}
function shareStorage() {
    try {
        const w = window || self;
        if (w && !w._iconifyStorage) w._iconifyStorage = {
...
function getStorage(provider, prefix) {
    if (storage$1[provider] === void 0) storage$1[provider] = /* @__PURE__ */ Object.create(null);
    const providerStorage = storage$1[provider];
    if (providerStorage[prefix] === void 0) providerStorage[prefix] = newStorage(provider, prefix);
...
function listIcons(provider, prefix) {
    let allIcons = [];
    let providers;
    if (typeof provider === "string") providers = [
        provider
    ];
    else providers = Object.keys(storage$1);
    providers.forEach((provider2)=>{
        let prefixes;
        if (typeof provider2 === "string" && typeof prefix === "string") prefixes = [
            prefix
        ];
        else prefixes = storage$1[provider2] === void 0 ? [] : Object.keys(storage$1[provider2]);
...
function addIcon(name, data) {
    ...
    if (!icon) return false;
    const storage1 = getStorage(icon.provider, icon.prefix);
    return addIconToStorage(storage1, icon.name, data);
}
function addCollection(data, provider) {
    if (typeof data !== "object") return false;
    if (typeof provider !== "string") provider = typeof data.provider === "string" ? data.provider : "";
    if (simpleNames && provider === "" && (typeof data.prefix !== "string" || data.prefix === "")) {
        let added = false;
        if (quicklyValidateIconSet(data)) {
            data.prefix = "";
            parseIconSet(data, (name, icon)=>{
                if (icon && addIcon(name, icon)) added = true;
            });
        }
        return added;
    }
    ...
    const storage3 = getStorage(provider, data.prefix);
...
function mergeCustomisations(defaults2, item) {
    ...
    for(const key in defaults2){
        ...
        switch(attr){
            ...
            case "hFlip":
            case "vFlip":
                if (value === true) result[attr] = !result[attr];
                break;
            case "hAlign":
            case "vAlign":
                if (typeof value === "string" && value !== "") result[attr] = value;
                break;
            case "width":
            case "height":
                if (typeof value === "string" && value !== "" || typeof value === "number" && value || value === null) result[attr] = value;
                break;
            case "rotate":
                if (typeof value === "number") result[attr] += value;
                break;
        }
    }
    return result;
...
function iconToSVG(icon, customisations) {
    ...
    if (width === "auto") width = box.width;
    if (height === "auto") height = box.height;
...
function addAPIProvider(provider, customConfig) {
    const config1 = createAPIConfig(customConfig);
    if (config1 === null) return false;
    configStorage[provider] = config1;
    return true;
}
...
const prepare = (provider, prefix, icons)=>{
    ...
    if (maxLength === void 0) maxLength = calculateMaxLength(provider, prefix);
    const type = "icons";
    let item = {
        type,
        provider,
...
function sortIcons(icons) {
    ...
    const storage4 = /* @__PURE__ */ Object.create(null);
    icons.sort((a, b)=>{
    ...
    icons.forEach((icon)=>{
        ...
        if (storage4[provider] === void 0) storage4[provider] = /* @__PURE__ */ Object.create(null);
        const providerStorage = storage4[provider];
        if (providerStorage[prefix] === void 0) providerStorage[prefix] = getStorage(provider, prefix);
        const localStorage = providerStorage[prefix];
        let list;
        if (localStorage.icons[name] !== void 0) list = result.loaded;
        else if (prefix === "" || localStorage.missing[name] !== void 0) list = result.missing;
...
function removeCallback(sources, id) {
    sources.forEach((source)=>{
        ...
        if (callbacks[provider] === void 0) return;
        const providerCallbacks = callbacks[provider];
        const prefix = source.prefix;
        const items = providerCallbacks[prefix];
        if (items) providerCallbacks[prefix] = items.filter((row)=>row.id !== id
        );
    });
}
function updateCallbacks(provider, prefix) {
    if (pendingUpdates[provider] === void 0) pendingUpdates[provider] = /* @__PURE__ */ Object.create(null);
    const providerPendingUpdates = pendingUpdates[provider];
    if (!providerPendingUpdates[prefix]) {
        providerPendingUpdates[prefix] = true;
        setTimeout(()=>{
            providerPendingUpdates[prefix] = false;
            if (callbacks[provider] === void 0 || callbacks[provider][prefix] === void 0) return;
            const items = callbacks[provider][prefix].slice(0);
            if (!items.length) return;
            const storage5 = getStorage(provider, prefix);
            ...
            items.forEach((item)=>{
                ...
                icons.pending = icons.pending.filter((icon)=>{
                    ...
                    const name = icon.name;
                    if (storage5.icons[name] !== void 0) icons.loaded.push({
                        provider,
                        prefix,
                        name
                    });
                    else if (storage5.missing[name] !== void 0) icons.missing.push({
                        provider,
                        prefix,
                        name
                    });
...
function storeCallback(callback, icons, pendingSources) {
    ...
    pendingSources.forEach((source)=>{
        const provider = source.provider;
        const prefix = source.prefix;
        if (callbacks[provider] === void 0) callbacks[provider] = /* @__PURE__ */ Object.create(null);
        const providerCallbacks = callbacks[provider];
        if (providerCallbacks[prefix] === void 0) providerCallbacks[prefix] = [];
        providerCallbacks[prefix].push(item);
    });
    return abort;
}
function listToIcons(list, validate = true, simpleNames1 = false) {
    const result = [];
    list.forEach((item)=>{
        const icon = typeof item === "string" ? stringToIcon(item, false, simpleNames1) : item;
        if (!validate || validateIcon(icon, simpleNames1)) result.push({
...
// src/query.ts
function sendQuery(config4, payload, query, done) {
    const resourcesCount = config4.resources.length;
    const startIndex = config4.random ? Math.floor(Math.random() * resourcesCount) : config4.index;
    let resources;
    if (config4.random) {
        let list = config4.resources.slice(0);
        resources = [];
        while(list.length > 1){
    ...
    } else resources = config4.resources.slice(startIndex).concat(config4.resources.slice(0, startIndex));
    ...
    function moduleResponse(item, response, data) {
        ...
        switch(status){
            case "pending":
                break;
            case "failed":
                if (isError || !config4.dataAfterTimeout) return;
                break;
            default:
                return;
        }
        ...
        clearQueue();
        if (!config4.random) {
            const index = config4.resources.indexOf(item.resource);
            if (index !== -1 && index !== config4.index) config4.index = index;
        }
        status = "completed";
        doneCallbacks.forEach((callback)=>{
            callback(data);
    ...
    function execNext() {
        ...
        queue.push(item);
        queriesSent++;
        timer = setTimeout(execNext, config4.rotate);
...
// src/index.ts
function setConfig(config5) {
    if (typeof config5 !== "object" || typeof config5.resources !== "object" || !(config5.resources instanceof Array) || !config5.resources.length) throw new Error("Invalid Reduncancy configuration");
    const newConfig = /* @__PURE__ */ Object.create(null);
    let key;
    for(key in defaultConfig)if (config5[key] !== void 0) newConfig[key] = config5[key];
    else newConfig[key] = defaultConfig[key];
    return newConfig;
}
function initRedundancy(cfg) {
    const config6 = setConfig(cfg);
    let queries = [];
    function cleanup() {
    ...
    function query(payload, queryCallback, doneCallback) {
        const query2 = sendQuery(config6, payload, queryCallback, (data, error)=>{
            cleanup();
...
function getRedundancyCache(provider) {
    if (redundancyCache[provider] === void 0) {
        const config7 = getAPIConfig(provider);
        if (!config7) return;
        const redundancy = initRedundancy(config7);
...
function sendAPIQuery(target, query, callback) {
    ...
    if (typeof target === "string") {
        ...
        const config8 = createAPIConfig(target);
        if (config8) {
            redundancy = initRedundancy(config8);
            const moduleKey = target.resources ? target.resources[0] : "";
            const api = getAPIModule(moduleKey);
            if (api) send1 = api.send;
        }
    }
    if (!redundancy || !send1) {
        callback(void 0, 424);
    ...
    return redundancy.query(query, send1, callback)().abort;
}
const cache = {};
...
function loadedNewIcons(provider, prefix) {
    if (loaderFlags[provider] === void 0) loaderFlags[provider] = /* @__PURE__ */ Object.create(null);
...
function loadNewIcons(provider, prefix, icons) {
    ...
    if (iconsToLoad[provider] === void 0) iconsToLoad[provider] = /* @__PURE__ */ Object.create(null);
    const providerIconsToLoad = iconsToLoad[provider];
    if (queueFlags[provider] === void 0) queueFlags[provider] = /* @__PURE__ */ Object.create(null);
    const providerQueueFlags = queueFlags[provider];
    if (pendingIcons[provider] === void 0) pendingIcons[provider] = /* @__PURE__ */ Object.create(null);
    const providerPendingIcons = pendingIcons[provider];
    if (providerIconsToLoad[prefix] === void 0) providerIconsToLoad[prefix] = icons;
    else providerIconsToLoad[prefix] = providerIconsToLoad[prefix].concat(icons).sort();
        ...
        setTimeout(()=>{
            ...
            params.forEach((item)=>{
                sendAPIQuery(provider, item, (data, error)=>{
                    const storage6 = getStorage(provider, prefix);
                    if (typeof data !== "object") {
                        if (error !== 404) return;
                        const t = Date.now();
                        item.icons.forEach((name)=>{
                            storage6.missing[name] = t;
                        });
                    } else try {
                        const parsed = addIconSet(storage6, data);
                        if (!parsed.length) return;
                        const pending = providerPendingIcons[prefix];
                        parsed.forEach((name)=>{
                            delete pending[name];
                        });
                        if (cache.store) cache.store(provider, data);
                    } catch (err2) {
                        console.error(err2);
                    }
...
const loadIcons = (icons, callback)=>{
    ...
    sortedIcons.pending.forEach((icon)=>{
        ...
        if (pendingIcons[provider] === void 0) pendingIcons[provider] = /* @__PURE__ */ Object.create(null);
        const providerPendingIcons = pendingIcons[provider];
        if (providerPendingIcons[prefix] === void 0) providerPendingIcons[prefix] = /* @__PURE__ */ Object.create(null);
        if (newIcons[provider] === void 0) newIcons[provider] = /* @__PURE__ */ Object.create(null);
        const providerNewIcons = newIcons[provider];
        if (providerNewIcons[prefix] === void 0) providerNewIcons[prefix] = [];
    });
    const time = Date.now();
    sortedIcons.pending.forEach((icon)=>{
        const provider = icon.provider;
    ...
    sources.forEach((source)=>{
        const provider = source.provider;
        const prefix = source.prefix;
        if (newIcons[provider][prefix].length) loadNewIcons(provider, prefix, newIcons[provider][prefix]);
    });
...
const loadIcon = (icon)=>{
    return new Promise((fulfill, reject)=>{
        ...
        loadIcons([
            ...
            if (loaded1.length && iconObj) {
                const storage7 = getStorage(iconObj.provider, iconObj.prefix);
                const data = getIconFromStorage(storage7, iconObj.name);
                if (data) {
                    fulfill(data);
                    return;
                }
...
function setCount(storage8, key, value) {
    try {
        storage8.setItem(countKey, value.toString());
        count[key] = value;
        return true;
    } catch (err) {
        return false;
...
function getCount(storage9) {
    const count2 = storage9.getItem(countKey);
    if (count2) {
...
function initCache(storage10, key) {
    try {
        storage10.setItem(versionKey, cacheVersion);
    } catch (err) {}
    setCount(storage10, key, 0);
}
function destroyCache(storage11) {
    try {
        const total = getCount(storage11);
        for(let i = 0; i < total; i++)storage11.removeItem(cachePrefix + i.toString());
    } catch (err) {}
}
const loadCache = ()=>{
    if (loaded) return;
    ...
    function load(key) {
        ...
        const getItem = (index)=>{
            ...
            try {
                ...
                if (typeof data !== "object" || typeof data.cached !== "number" || data.cached < minTime || typeof data.provider !== "string" || typeof data.data !== "object" || typeof data.data.prefix !== "string") valid = false;
                else {
                    const provider = data.provider;
                    const prefix = data.data.prefix;
                    const storage12 = getStorage(provider, prefix);
                    valid = addIconSet(storage12, data.data).length > 0;
                }
            } catch (err) {
                valid = false;
            }
            if (!valid) func.removeItem(name);
            return valid;
        };
        try {
            const version = func.getItem(versionKey);
            if (version !== cacheVersion) {
                if (version) destroyCache(func);
                initCache(func, key);
                return;
            }
            let total = getCount(func);
            for(let i = total - 1; i >= 0; i--)if (!getItem(i)) {
                if (i === total - 1) total--;
                else emptyList[key].push(i);
            }
            setCount(func, key, total);
        } catch (err) {}
    }
    for(const key1 in config)load(key1);
};
const storeCache = (provider, data)=>{
    ...
    function store(key) {
        ...
        if (index === void 0) {
            index = count[key];
            if (!setCount(func, key, index + 1)) return false;
        }
        try {
            const item = {
...
function toggleBrowserCache(storage13, value) {
    switch(storage13){
        case "local":
        case "session":
            config[storage13] = value;
            break;
        case "all":
            for(const key in config)config[key] = value;
            break;
    }
}
const separator = /[\s,]+/;
...
props)=>{
    ...
    for(let key in props){
        ...
        switch(key){
            ...
            // Alignment as string
            case 'align':
                if (typeof value === 'string') alignmentFromString(customisations, value);
                break;
            ...
            default:
                if (customisationAliases[key] !== void 0) {
                    // Aliases for customisations
                    if (customisationAliases[key].boolean && (value === true || value === 'true' || value === 1)) // Check for boolean
                    customisations[customisationAliases[key].attr] = true;
                    else if (!customisationAliases[key].boolean && typeof value === 'string' && value !== '') // String
                    customisations[customisationAliases[key].attr] = value;
                } else if (defaults[key] === void 0) // Copy missing property if it does not exist in customisations
    ...
    // Add icon stuff
    for(let key2 in item.attributes)componentProps[key2] = item.attributes[key2];
    if (item.inline && style.verticalAlign === void 0 && style['vertical-align'] === void 0) style.verticalAlign = '-0.125em';
    // Counter for ids based on "id" property to render icons consistently on server and client
    let localCounter = 0;
    let id = props.id;
    if (typeof id === 'string') // Convert '-' to '_' to avoid errors in animations
    id = id.replace(/-/g, '_');
    // Add innerHTML and style to props
    componentProps['innerHTML'] = replaceIDs(item.body, id ? ()=>id + 'ID' + localCounter++
     : 'iconifyVue');
    if (Object.keys(style).length > 0) componentProps['style'] = style;
    // Render icon
    return _vue.h('svg', componentProps);
};
/**
 * Enable cache
 */ function enableCache(storage14) {
    toggleBrowserCache(storage14, true);
}
/**
 * Disable cache
 */ function disableCache(storage15) {
    toggleBrowserCache(storage15, false);
}
/**
 * Initialise stuff
 */ // Enable short names
    ...
    if (_window.IconifyPreload !== void 0) {
        ...
        if (typeof preload === 'object' && preload !== null) (preload instanceof Array ? preload : [
            ...
            try {
                ...
                !addCollection(item)) console.error(err);
            } catch (e) {
                console.error(err);
            }
        });
...
const Icon = _vue.defineComponent({
...
},{"@vue/runtime-dom":"9wNvI","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"53GNG":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "render", ()=>render
);
var _vue = require("vue");
var _parcel2XPng = require("../assets/parcel@2x.png");
var _parcel2XPngDefault = parcelHelpers.interopDefault(_parcel2XPng);
const _withScopeId = (n)=>(_vue.pushScopeId("data-v-0393db"), n = n(), _vue.popScopeId(), n)
;
const _hoisted_1 = {
    id: "mynav"
};
const _hoisted_2 = /*#__PURE__*/ _withScopeId(()=>/*#__PURE__*/ _vue.createElementVNode("div", {
        class: "logo"
    }, [
        /*#__PURE__*/ _vue.createElementVNode("img", {
            src: _parcel2XPngDefault.default
        })
    ], -1 /* HOISTED */ )
);
const _hoisted_3 = [
...
function render(_ctx, _cache) {
    const _component_Icon = _vue.resolveComponent("Icon");
    return _vue.openBlock(), _vue.createElementBlock("header", _hoisted_1, [
        _hoisted_2,
        _vue.createElementVNode("ul", null, [
            (_vue.openBlock(true), _vue.createElementBlock(_vue.Fragment, null, _vue.renderList(_ctx.stuff, (n)=>{
                return _vue.openBlock(), _vue.createElementBlock("li", {
                    ...
                    _vue.createVNode(_component_Icon, {
                        icon: n.ico,
                        height: "28"
                    }, null, 8 /* PROPS */ , [
                        "icon"
                    ...
                    _vue.createElementVNode("a", {
                        href: n.page
                    }, _vue.toDisplayString(n.name), 9 /* TEXT, PROPS */ , _hoisted_3)
                ], 2 /* CLASS */ );
            }), 128 /* KEYED_FRAGMENT */ ))
        ])
    ]);
...
if (module.hot) module.hot.accept(()=>{
    __VUE_HMR_RUNTIME__.rerender('0393db-hmr', render);
});

},{"vue":"gzxs9","../assets/parcel@2x.png":"4nbT8","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4nbT8":[function(require,module,exports) {
module.exports = require('./helpers/bundle-url').getBundleURL('wLxnD') + "parcel@2x.8b596042.png" + "?" + Date.now();
...
},{}],"bFWqG":[function() {},{}],"4YZm1":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let NOOP = ()=>{};
exports.default = (script)=>{};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"lV0KE":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let script;
let initialize = ()=>{
    script = require('script:./Entry.vue');
    if (script.__esModule) script = script.default;
    script.render = require('template:./Entry.vue').render;
    script.__cssModules = require('style:./Entry.vue').default;
    require('custom:./Entry.vue').default(script);
    script.__scopeId = 'data-v-9e431c';
    script.__file = "/home/vaman/apps/2022/parcelApp/src/components/Entry.vue";
};
initialize();
if (module.hot) {
    script.__hmrId = '9e431c-hmr';
    module.hot.accept(()=>{
        setTimeout(()=>{
            initialize();
            if (!__VUE_HMR_RUNTIME__.createRecord('9e431c-hmr', script)) __VUE_HMR_RUNTIME__.reload('9e431c-hmr', script);
        }, 0);
    });
}
exports.default = script;

},{"script:./Entry.vue":"76dLD","template:./Entry.vue":"5KAdu","style:./Entry.vue":"592xh","custom:./Entry.vue":"e4PaP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"76dLD":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _vue = require("@iconify/vue");
exports.default = {
    ...
    computed: {
        iconame () {
            return "tabler:" + this.icos;
        }
    }
};

},{"@iconify/vue":"d1SnY","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5KAdu":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "render", ()=>render
);
var _vue = require("vue");
const _withScopeId = (n)=>(_vue.pushScopeId("data-v-9e431c"), n = n(), _vue.popScopeId(), n)
;
const _hoisted_1 = {
    id: "v-logo",
    ref: "v_logo"
    ...
    return _vue.openBlock(), _vue.createElementBlock("div", _hoisted_1, [
        _vue.createElementVNode("div", _hoisted_2, [
            _vue.createElementVNode("div", _hoisted_3, [
                _vue.createElementVNode("h1", null, _vue.toDisplayString(_ctx.title), 1 /* TEXT */ )
            ]),
...
if (module.hot) module.hot.accept(()=>{
    __VUE_HMR_RUNTIME__.rerender('9e431c-hmr', render);
});

},{"vue":"gzxs9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"592xh":[function() {},{}],"e4PaP":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let NOOP = ()=>{};
exports.default = (script)=>{};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"3fsRG":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let script;
let initialize = ()=>{
    script = require('script:./cbtn.vue');
    if (script.__esModule) script = script.default;
    script.render = require('template:./cbtn.vue').render;
    script.__cssModules = require('style:./cbtn.vue').default;
    require('custom:./cbtn.vue').default(script);
    script.__scopeId = 'data-v-f9aeac';
    script.__file = "/home/vaman/apps/2022/parcelApp/src/components/cbtn.vue";
};
initialize();
if (module.hot) {
    script.__hmrId = 'f9aeac-hmr';
    module.hot.accept(()=>{
        setTimeout(()=>{
            initialize();
            if (!__VUE_HMR_RUNTIME__.createRecord('f9aeac-hmr', script)) __VUE_HMR_RUNTIME__.reload('f9aeac-hmr', script);
        }, 0);
    });
}
exports.default = script;

},{"script:./cbtn.vue":"dEeTd","template:./cbtn.vue":"46APv","style:./cbtn.vue":"3Fxy8","custom:./cbtn.vue":"29NGZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dEeTd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
exports.default = {
    props: {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"46APv":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "render", ()=>render
);
...
function render(_ctx, _cache) {
    return _vue.openBlock(), _vue.createElementBlock("button", {
        ...
        onClick: _cache[0] || (_cache[0] = ($event)=>_ctx.cnt++
        )
    }, _vue.toDisplayString(_ctx.cnt), 1 /* TEXT */ );
}
if (module.hot) module.hot.accept(()=>{
    __VUE_HMR_RUNTIME__.rerender('f9aeac-hmr', render);
});

},{"vue":"gzxs9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"3Fxy8":[function() {},{}],"29NGZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
let NOOP = ()=>{};
exports.default = (script)=>{};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7SfAk":[function() {},{}]},["a6ObB","75iMF"], "75iMF", "parcelRequire129b")

//# sourceMappingURL=market.813167df.js.map
